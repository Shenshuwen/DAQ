...
用 Keil MDK 打开 `29_A/DAQ/MDK-ARM/DAQ.uvprojx`。
...
APP 链接地址: `0x08004000`, 大小 `0xBC00` (47 KB)。

### Bootloader (29_B)

用 Keil MDK 打开 `29_B/DAQ_B/MDK-ARM/DAQ_B.uvprojx`。

编译产物:

- `DAQ_B.hex` (用于烧录)

Bootloader 链接地址: `0x08000000`, 大小 `0x4000` (16 KB)。

### 烧录顺序

1. 先烧录 Bootloader (`DAQ_B.hex`) 到 `0x08000000`
2. 再烧录 APP (`DAQ.hex`) 到 `0x08004000`

或合并两个 HEX 一次烧录。

## 半实物测试工具

位于 `tools/daq_hw_test/`, 用于在无真实传感器时验证完整工作流。
...
# 循环读取
python tools/daq_hw_test/daq_hw_test.py --data-port COM8 --loop --interval 1
...
详见 [tools/daq_hw_test/README.md](tools/daq_hw_test/README.md)。
...
- 引脚: PA9 (TX), PA10 (RX)
- 波特率: 115200 8N1
- 连接 CH340 或 USB-TTL 模块
- APP 侧由 `app_config.h` 中 `SENSOR_UART1_PRINT_ENABLE` 控制

### 传感器模拟

无真实 SPI 传感器时, 在 `29_A/DAQ/Application/Inc/app_config.h` 中设置:

```c
#define SENSOR_SIM_ENABLE 1
```

此时 APP 内部生成 8 路模拟输入寄存器 (CH0–CH7 递增), 不走 SPI。
